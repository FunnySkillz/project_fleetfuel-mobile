...
export function normalizeBackupPreferences(value: unknown): AppPreferences {
  const fallback: AppPreferences = { themeMode: 'system', language: 'en', appLockEnabled: false, appMode: 'local' };
  ...
  return {
    ...
    appMode: record.appMode === 'local' || record.appMode === 'shared' ? record.appMode : fallback.appMode,
  };
}
