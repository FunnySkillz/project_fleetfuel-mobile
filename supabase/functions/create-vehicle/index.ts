...
Deno.serve(async (request) => {
  ...
  const plate = typeof payload.plate === 'string' ? payload.plate.trim() : '';

  if (!fleetId || !name || !plate) {
    return error('validation_error', 'fleetId, name, and plate are required.', 400);
  }

  const supabase = createServiceRoleClient();
  const { data, error: rpcError } = await supabase.rpc('shared_create_vehicle', {
    p_fleet_id: fleetId,
    p_name: name,
    p_plate: plate,
