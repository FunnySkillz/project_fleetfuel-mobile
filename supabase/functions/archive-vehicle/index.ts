...
Deno.serve(async (request) => {
  ...
  const archiveReason = typeof payload.archiveReason === 'string' ? payload.archiveReason.trim() : '';

  if (!fleetId || !vehicleId) {
    return error('validation_error', 'fleetId and vehicleId are required.', 400);
  }

  const supabase = createServiceRoleClient();
  const { data, error: rpcError } = await supabase.rpc('shared_archive_vehicle', {
    p_fleet_id: fleetId,
